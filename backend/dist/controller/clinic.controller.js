...
import { sendBulkMultiChannelNotification, sendMultiChannelNotification } from "../utils/send-notification.js";
    ...
    try {
        ...
        if (nearbyDoctors.length > 0) {
            ...
            if (recipients.length > 0) {
                await sendBulkMultiChannelNotification(recipients, "New Requirement", `${title} requirement posted near you`, { requirementId: requirement.id });
            }
        }
...
export const acceptPitch = async (req, res) => {
    try {
        ...
        if (pitch.doctor.notificationToken || pitch.doctor.email || pitch.doctor.phoneNumber) {
            ...
            }, `Pitch Accepted`, `Your application has been accepted for ${pitch.jobRequirement.title}`, { pitchId: pitch.id });
        }
