...
export class QueryBuilder {
    ...
    // Paginated queries
    static async getPaginatedClinics(options = {}) {
    ...
    static async getPaginatedDoctors(options = {}) {
        const page = options.page || 1;
        const limit = options.limit || 10;
        const skip = (page - 1) * limit;
    ...
    static async getPaginatedJobRequirements(clinicId, options = {}) {
        const page = options.page || 1;
        const limit = options.limit || 10;
        const skip = (page - 1) * limit;
    ...
    static async getClinicDashboardOverview(clinicId) {
        const [totalRequirements, requirementsByStatus, totalPitches, pitchesByStatus, recentPitches, totalAccepted] = await Promise.all([
    ...
    static async getDoctorDashboardOverview(doctorId) {
        const [totalApplications, applicationsByStatus, totalAccepted, recentApplications] = await Promise.all([
            prisma.pitch.count({
                where: { doctorId }
            }),
            prisma.pitch.groupBy({
