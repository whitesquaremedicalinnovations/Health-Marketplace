...
export const getClinicsByLocation = async (req: Request, res: Response) => {
    try {
        const { lat, lng, radius, sortBy, location } = req.query as { 
            lat?: string, 
            lng?: string, 
            radius?: string, 
            sortBy?: string, 
        ...
        if (location) {
            clinics = clinics.filter(clinic => 
                clinic.clinicAddress.toLowerCase().includes(location.toLowerCase()) ||
...
export const getRequirementsByLocation = async (req: Request, res: Response) => {
    try {
        const { lat, lng, radius, sortBy, specialization, type, search, doctorId, experienceLevel } = req.query as { 
            lat?: string, 
            lng?: string, 
            radius?: string, 
            sortBy?: string,
        ...
        if (specialization && specialization !== 'all') {
            ...
            requirements = requirements.filter(req => 
                req.specialization && specializations.includes(req.specialization)
            );
        }
...
export const pitchRequirement = async (req: Request, res: Response) => {
    try {
...
export const getMyPitches = async (req: Request, res: Response) => {
    try {
...
export const withdrawPitch = async (req: Request, res: Response) => {
    try {
...
export const getMyAcceptedPitches = asyncHandler(async (req: Request, res: Response) => {
    ...
    const uniqueClinicsMap = new Map<string, typeof acceptedWork[0]>();
...
export const getAllDoctors = async (req: Request, res: Response) => {
    try {
        const { sortBy, location } = req.query as { sortBy?: string, location?: string };
