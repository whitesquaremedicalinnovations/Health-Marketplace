import { sendEmailNotification } from './send-email.ts';
import { sendWhatsAppNotification } from './send-whatsapp.ts';
 ...
 * @param data - Additional data for push notifications
 */
export async function sendBulkMultiChannelNotification(
    ...
    const emailNotifications: Array<{ to: string; subject: string; html: string }> = [];
    const whatsappNotifications: Array<{ to: string; message: string }> = [];
    ...
    if (pushTokens.length > 0) {
        promises.push(
            sendBulkPushNotification(pushTokens, title, body, data)
    ...
    if (emailNotifications.length > 0) {
        promises.push(
            Promise.allSettled(emailNotifications.map(email => sendEmailNotification(email)))
    ...
    if (whatsappNotifications.length > 0) {
        promises.push(
            Promise.allSettled(whatsappNotifications.map(whatsapp => sendWhatsAppNotification(whatsapp)))
