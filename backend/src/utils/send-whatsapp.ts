...
export async function sendWhatsAppNotification({ to, message }: WhatsAppNotificationParams) {
  try {
    if (!twilioClient) {
...
export async function sendBulkWhatsAppNotifications(notifications: WhatsAppNotificationParams[]) {
  try {
    if (!twilioClient) {
    ...
    const results = await Promise.allSettled(
      notifications.map(notification => sendWhatsAppNotification(notification))
    );
