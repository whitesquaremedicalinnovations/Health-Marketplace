...
export default function ChatClient() {
  ...
  // Handle direct patient navigation from dashboard
  useEffect(() => {
    const patientId = searchParams.get('patient');
    if (patientId && connectedDoctors.length > 0 && patients.length > 0) {
      const targetPatient = patients.find(p => p.id === patientId);
      if (targetPatient) {
        handlePatientSelect(targetPatient);
      }
    }
  }, [searchParams, connectedDoctors, patients]);

  // Auto-select first doctor and load patients when navigating with patient ID
  useEffect(() => {
    const patientId = searchParams.get('patient');
    if (patientId && connectedDoctors.length > 0 && !selectedDoctor) {
      // Find which doctor has this patient
      const findDoctorForPatient = async () => {
        for (const doctor of connectedDoctors) {
          try {
            ...
            const doctorPatients = (allPatients || []).filter((patient: Patient) =>
              patient.assignedDoctors.some(d => d.id === doctor.id)
            );
            
            if (doctorPatients.some((p: Patient) => p.id === patientId)) {
              handleDoctorSelect(doctor);
              break;
            }
          } catch (error) {
            console.error("Error finding doctor for patient:", error);
          }
        }
      };
  ...
  }, [searchParams, connectedDoctors, userId, selectedDoctor]);
