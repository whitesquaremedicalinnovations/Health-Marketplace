...
export default function RequirementsScreen() {
    ...
    if (user?.id) {
      try {
        ...
        if (selectedCoordinates) {
          params.lat = selectedCoordinates.lat;
          params.lng = selectedCoordinates.lng;
        }
  ...
  }, [user, radius, selectedCoordinates, selectedSpecializations, typeFilter, searchTerm, sortBy]);
  ...
  useEffect(() => {
    ...
    // Apply client-side filters for verified clinics and experience levels
    if (verifiedOnly) {
  ...
  const handleLocationSelect = (place: PlaceResult | null, coordinates?: {lat: number, lng: number}) => {
  ...
  const renderSortButton = (label: string, value: string, isActive: boolean, onPress: () => void) => (
    <TouchableOpacity
      onPress={onPress}
  ...
  const renderSpecializationChip = (spec: string, isSelected: boolean) => (
    <TouchableOpacity
      key={spec}
      onPress={() => {
        if (isSelected) {
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#f8fafc' }}>
      ...
      >
        ...
        <View style={{ padding: 20, paddingTop: 30 }}>
              ...
              <View className="border-t border-gray-100 pt-4">
                ...
                >
                  <View className="flex-row items-center">
                    ...
                    <Text className="text-gray-700 font-medium ml-2">Advanced Filters</Text>
                  </View>
                  {showAdvancedFilters ? (
                  ...
                  <View className="mt-4 space-y-4">
                    ...
                    <View>
                      ...
                      {selectedLocation && (
                        <View className="flex-row items-center mt-2">
                          ...
                          <Text className="text-blue-600 text-sm ml-1 flex-1">{selectedLocation}</Text>
                          <TouchableOpacity
                            onPress={() => {
                              setSelectedLocation('');
                              setSelectedCoordinates(null);
                    ...
                    {selectedCoordinates && (
                      <View>
                        <View className="flex-row items-center justify-between mb-2">
                          <Text className="text-gray-700 font-medium">Search Radius</Text>
                        ...
                        <View className="flex-row justify-between">
                          <Text className="text-gray-500 text-xs">5 km</Text>
                          <Text className="text-gray-500 text-xs">100 km</Text>
                        </View>
                      </View>
                    )}
                    ...
                    <View>
                      ...
                      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                        {renderSortButton('Latest', 'date_desc', sortBy === 'date_desc', () => setSortBy('date_desc'))}
                        {renderSortButton('Oldest', 'date_asc', sortBy === 'date_asc', () => setSortBy('date_asc'))}
                        {renderSortButton('Most Applied', 'applications_desc', sortBy === 'applications_desc', () => setSortBy('applications_desc'))}
                        {renderSortButton('Least Applied', 'applications_asc', sortBy === 'applications_asc', () => setSortBy('applications_asc'))}
                      </ScrollView>
                    </View>
                    ...
                    <View>
                      ...
                      <View className="flex-row flex-wrap">
                        {SPECIALIZATIONS.map(spec => 
                          renderSpecializationChip(spec, selectedSpecializations.includes(spec))
                        )}
                      </View>
                    </View>
          ...
          {(searchTerm || typeFilter !== 'all' || verifiedOnly || selectedLocation || selectedSpecializations.length > 0) && (
            <View className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-4">
              <Text className="text-blue-800 font-medium mb-2">Active Filters:</Text>
              <View className="flex-row flex-wrap gap-2">
                {searchTerm && (
                  <View className="bg-blue-100 rounded-full px-3 py-1">
                    <Text className="text-blue-800 text-xs">Search: "{searchTerm}"</Text>
                  </View>
                )}
                {typeFilter !== 'all' && (
                  <View className="bg-blue-100 rounded-full px-3 py-1">
                    <Text className="text-blue-800 text-xs">Type: {formatJobType(typeFilter)}</Text>
                  </View>
                )}
                {verifiedOnly && (
                  <View className="bg-blue-100 rounded-full px-3 py-1">
                    <Text className="text-blue-800 text-xs">Verified Only</Text>
                  </View>
                )}
                {selectedLocation && (
                  <View className="bg-blue-100 rounded-full px-3 py-1">
                    <Text className="text-blue-800 text-xs">Location: {selectedLocation}</Text>
                  </View>
                )}
